...
function initHash() {
  ...
  document.getElementById('hash-generate').addEventListener('click', async () => {
    ...
    if (!text) { output.textContent = 'Enter text to hash.'; return; }
    ...
    try {
      if (algo === 'MD5') {
        ...
        output.textContent = hashArray.map(b => b.toString(16).padStart(2, '0')).join('');
      }
    } catch (e) {
...
function md5(string) {
  ...
  function rhex(n) { var s = '', j; for (j = 0; j < 4; j++) s += ('0' + ((n >> (j * 8 + 4)) & 0x0F).toString(16)) + ((n >> (j * 8)) & 0x0F).toString(16); return s; }
  ...
  for (i = 0; i < strl; i++) tail[i >> 2] |= string.charCodeAt(i) << ((i % 4) << 3);
  tail[i >> 2] |= 0x80 << ((i % 4) << 3);
